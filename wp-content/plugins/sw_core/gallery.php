...
class Ya_Gallery_Shortcode{
	...
	function gallery( $attr ){
		...
		if($type == 'column'){
		wp_register_script( 'ya_gallery', plugins_url( '/js/ya_gallery.js', __FILE__ ),array(), null, true );
		if (is_feed()) {
		$output = "\n";
	...
	return $output;
			...
			/** slide **/
		if($type == 'slide'){
		wp_register_script( 'ya_slidegallery', plugins_url( '/js/slidegallery.js', __FILE__ ),array(), null, true );
		...
		/** flex **/
		if($type == 'flex'){
		wp_register_script( 'ya_flexgallery', plugins_url( '/js/flexgallery.js', __FILE__ ),array(), null, true );
		...
		$html ='<div class ="gallery-images '.$class.'"><div id="flexslider-gallery" class="flexslider ">';
	        ...
	        $html .='<div style="overflow: hidden; position: relative;" class="flex-viewport"><ul class="slides">';
			foreach ($attachments as $attachment){
		    $html .='<li><a href="'.wp_get_attachment_url( $attachment->ID).'" class="product_image" title="19">'.wp_get_attachment_image($attachment->ID, 'medium').	'</a></li>';
			}
			$html .='</ul></div>';
