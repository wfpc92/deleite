 ...
 */
function sw_paradise_customize_register( $wp_customize ){

	$priority = 200;
	$sections = array();
	$sw_paradise_options = sw_paradise_options();	
	$i = 0;
	foreach ( $sw_paradise_options->sections as $section) {
		...
		if ( isset($section['fields']) && is_array($section['fields']) ) {
			...
			foreach ($section['fields'] as $field ){
				...
				if ($sw_paradise_options->get($customize)) {
					...
					$field['settings'] = $sw_paradise_options->args['opt_name'].'['.$field['id'].']';
	...
	foreach ( $sections as $section ) {
		...
		foreach ($section['fields'] as $field ){ 
			...
			$wp_customize->add_setting( 
					$sw_paradise_options->args['opt_name'].'['.$field['id'].']', 
					array(
						'default'  	=> $sw_paradise_options->get($field['id']),
    					'type' 		=> 'option',
...
add_action( 'customize_register', 'sw_paradise_customize_register' );
